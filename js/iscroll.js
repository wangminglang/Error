...
// INSERT POINT: _init
	...
	_move: function (e) {
		...
		if(this.options.slideLeftRight){
			this.deltaX = deltaX;
			this._execEvent('slideLeftRight');
		}
		...
		if(target && utils.hasClass(target,"iScroll-instance")){
			iscroll = target.iscroll;
			isY = iscroll.y;
			maxY = iscroll.maxScrollY - isY;
			
			if(target !== scroller && ( (isY < 0 && maxY < 0) || (this.directionLocked == 'h' && this.options.snap))){
...
/* REPLACE END: _translate */
	...
	if ( indicators && this.options.indicatorAnimate === true) {
		for ( var i = indicators.length; i--; ) {
			indicators[i].updatePosition();
		}
	}
...
// INSERT POINT: _wheel
	...
	_initSnap: function () {
		...
		if(this.options.indicatorAnimate === false){
			this.on("scrollEnd",function(){
				var indicators = this.indicators;
				if ( indicators ) {
					for ( var i = indicators.length; i--; ) {
